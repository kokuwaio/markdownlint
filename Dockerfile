...
RUN --mount=type=cache,target=/build,id=nodejs \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v24.15.0/node-v24.15.0-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v24.15.0/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v24.15.0/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt && \
	tar --xz --extract --file="node-v24.15.0-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v24.15.0-linux-$ARCH" /opt/node && \
...
RUN --mount=type=tmpfs,target=/tmp PATH="$PATH:/opt/node/bin" npm install "markdownlint-cli@0.48.0" --global --no-fund --cache=/tmp && \
...
FROM docker.io/library/debian:13.4-slim@sha256:4ffb3a1511099754cddc70eb1b12e50ffdb67619aa0ab6c13fcd800a78ef7c7a
