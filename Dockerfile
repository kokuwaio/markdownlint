# hadolint global ignore=DL3008,SC3010
FROM docker.io/library/debian:13.7-slim@sha256:a99cfc517144bc59b1978475ec53b46ecabec7e43635402ee5b77cc54cd1b20a AS base
...
RUN --mount=type=cache,target=/build,id=nodejs \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v26.10.0/node-v26.10.0-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v26.10.0/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v26.10.0/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt && \
	tar --xz --extract --file="node-v26.10.0-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v26.10.0-linux-$ARCH" /opt/node && \
