...
RUN --mount=type=cache,target=/build \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v24.9.0/node-v24.9.0-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v24.9.0/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v24.9.0/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt && \
	tar --xz --extract --file="node-v24.9.0-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v24.9.0-linux-$ARCH" /opt/node && \
...
FROM docker.io/library/debian:13.1-slim@sha256:c2880112cc5c61e1200c26f106e4123627b49726375eb5846313da9cca117337
