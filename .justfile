...
@build:
	docker build . --tag=kokuwaio/markdownlint:dev --build-arg=NPM_CONFIG_REGISTRY --load

# Inspect image with docker.
@inspect: build
	docker image inspect kokuwaio/markdownlint:dev

# Inspect image layers with `dive`.
@dive: build
	docker run --rm -it --volume=/var/run/docker.sock:/var/run/docker.sock:ro wagoodman/dive:latest kokuwaio/markdownlint:dev
